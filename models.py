...
class RNet(nn.Module):
    ...
    def forward(self, Pw, Pc, Qw, Qc, device):
        lp = Pw.size()[0]
        lq = Qw.size()[0]
        P = torch.cat([Pw, Pc], dim=2)
        Q = torch.cat([Qw, Qc], dim=2)
        Up = self.encoder(P, device)
        Uq = self.encoder(Q, device)
        v = self.pqmatcher(Up, Uq, device)
        torch.cuda.empty_cache()
        h = self.selfmatcher(v, device)
        p1, p2 = self.pointer(h, Uq, device)
        return p1, p2